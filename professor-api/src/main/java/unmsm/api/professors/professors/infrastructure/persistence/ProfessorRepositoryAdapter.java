...
public class ProfessorRepositoryAdapter implements ProfessorRepositoryPort {
    ...
    @Autowired
    public ProfessorRepositoryAdapter(ProfessorMongoRepository professorMongoRepository, ProfessorMongoMapper professorMongoMapper) {
    ...
    public List<Professor> findAll() {
        return professorMongoRepository.findAll()
                .stream()
    ...
    public Optional<Professor> findById(String id) {
        return professorMongoRepository.findById(id)
                .map(professorMongoMapper::toModel);
    }

    @Override
    public List<Professor> findByFaculty(String faculty) {
        return professorMongoRepository.findByFaculty(faculty)
                .stream()
                .map(professorMongoMapper::toModel)
                .toList();
    }
    ...
    public List<Professor> findByName(String name) {
        return professorMongoRepository.findByName(name)
                .stream()
                .map(professorMongoMapper::toModel)
                .toList();
    }
